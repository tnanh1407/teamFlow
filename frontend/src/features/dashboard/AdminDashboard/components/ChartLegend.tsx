...
export default function ChartLegend({ payload }: { payload?: ChartLegendEntry[] }) {
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-xs">
      {payload.map((entry) => (
        <div key={entry.value} className="flex items-center gap-2 text-zinc-600 dark:text-zinc-300">
          <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: entry.color ?? "var(--chart-1)" }} />
