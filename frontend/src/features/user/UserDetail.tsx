    ...
    function FormComponent() {
      ...
      return (
        <div className="space-y-3" style={{ textAlign: "left" }}>
          ...
          <div>
            <label className={labelClass}>Mật khẩu <span className="text-zinc-400 font-normal">(để trống nếu không đổi)</span></label>
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={f.password} onChange={(e) => setF((p) => ({ ...p, password: e.target.value }))} placeholder="Để trống nếu không đổi" className={inputClass} />
              <button type="button" onClick={() => setShowPassword((p) => !p)} className="absolute right-2 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-zinc-600 cursor-pointer border-none bg-transparent text-xs">{showPassword ? "Ẩn" : "Hiện"}</button>
            </div>
          </div>
          <div>
            <label className={labelClass}>Vai trò</label>
            <div className="relative">
              <button type="button" onClick={() => setPosOpen((p) => !p)} className={`${inputClass} flex items-center justify-between`}>
                <span>{positionOptions.find((o) => o.value === f.position)?.label ?? f.position}</span>
              ...
              {posOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 rounded-lg border border-zinc-200 bg-white shadow-lg z-10 overflow-hidden">
                  {availablePositions.map((p) => (
                    <button key={p.value} type="button" onClick={() => { setF((prev) => ({ ...prev, position: p.value })); setPosOpen(false) }} className={`w-full text-left px-3 py-2 text-sm hover:bg-zinc-50 transition cursor-pointer border-none ${f.position === p.value ? "bg-blue-50 text-blue-700 font-medium" : "text-zinc-700"}`}>{p.label}</button>
                  ))}
                </div>
              )}
            </div>
